...
module.exports = app => {
  class RoomService extends app.Service {
    ...
    async roomList(start, size, order) {
      ...
      let sql;
      if (order === 1) {
        sql = `SELECT ${field} FROM data_room ORDER BY room_hits DESC LIMIT ${app.mysql.escape(start)},${app.mysql.escape(size)}`;
      } else {
        sql = `SELECT ${field} FROM data_room ORDER BY room_id DESC LIMIT ${app.mysql.escape(start)},${app.mysql.escape(size)}`;
      }
