module.exports = app => {
  class VeCodeService extends app.Service {
    ...
    async update(mobile, code, state) {
      const sql = 'UPDATE data_sms SET sms_status = ' + app.mysql.escape(state) + ' WHERE sms_mobile = ' + app.mysql.escape(mobile) + ' AND sms_content=' + app.mysql.escape(code);
    ...
    async find(mobile, code) {
      const sql = 'SELECT COUNT(*) as total FROM data_sms WHERE sms_mobile=' + app.mysql.escape(mobile) + ' AND sms_content=' + app.mysql.escape(code) + ' AND TIMEDIFF(NOW(),sms_create_time) <= 60 AND sms_status = 0';
    ...
    async findByMobile(mobile) {
      const sql = 'SELECT COUNT(*) as total FROM data_sms WHERE sms_mobile=' + app.mysql.escape(mobile) + ' AND TIMEDIFF(NOW(),sms_create_time) <= 600 AND sms_status = 0';
      const result = await app.mysql.query(sql);
      return result[0].total > 0;
    }
