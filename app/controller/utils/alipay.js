...
const AlipayConfig = {
  ...
  partner: '2088521292039043',

  // 交易安全检验码，由数字和字母组成的32位字符串
  key: 'qtddpb2rv10qfeeth16k3zs3odi8kw0y',

  // 签约支付宝账号或卖家收款支付宝帐户
  seller_email: 'fantongkeji@163.com',
...
// 获取验证码
const getMySign = function(params) {
...
const requestUrl = function(host, path, callback) {
  ...
  verity: function(params, callback) {
    ...
    if (mysign === sign) {
      ...
      if (params['notify_id']) {
        ...
        const partner = AlipayConfig.partner;
        const veryfy_path = AlipayConfig.HTTPS_VERIFY_PATH + 'partner=' + partner + '&notify_id=' + params['notify_id'];
...
exports.alipayto = function(req = '') {
  ...
  // 请与贵网站订单系统中的唯一订单号匹配
  const out_trade_no = '20120708132324';
  // 订单名称，显示在支付宝收银台里的“商品名称”里，显示在支付宝的交易管理的“商品名称”的列表里。
  // const subject = req.body.subject;
  const subject = '商品名称';
  // 订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
  // const body = req.body.alibody;
  const body = '商品描述';
  ...
  // 防钓鱼时间戳
  const anti_phishing_key = '';
  // 获取客户端的IP地址，建议：编写获取客户端IP地址的程序
  const exter_invoke_ip = '';
  // 注意：
  // 1.请慎重选择是否开启防钓鱼功能
  ...
  const extra_common_param = '';
  // 默认买家支付宝账号
  const buyer_email = '';
  // 商品展示地址，要用http:// 格式的完整路径，不允许加?id=123这类自定义参数
  const show_url = 'http://www.xxx.com/order/myorder.jsp';
  ...
  /* *
     ...
     * @return 表单提交HTML信息
     */
  const create_direct_pay_by_user = function(sParaTemp) {
    // 增加基本配置
    sParaTemp.push(['service', 'alipay.wap.create.direct.pay.by.user']);
    sParaTemp.push(['partner', AlipayConfig.partner]);
    sParaTemp.push(['seller_id', AlipayConfig.partner]);
    ...
    /* *
         ...
         * @return 提交表单HTML文本
         */
    const buildURL = function(sParaTemp) {
      /* *
             * 生成要请求给支付宝的参数数组
             * @param sParaTemp 请求前的参数数组
             * @return 要请求的参数数组
             */
      const buildRequestPara = function(sParaTemp) {
        const sPara = [];
        // 除去数组中的空值和签名参数
        for (let i1 = 0; i1 < sParaTemp.length; i1++) {
          const value = sParaTemp[i1];
        ...
        prestr = prestr + AlipayConfig.key; // 把拼接后的字符串再与安全校验码直接连接起来
        const crypto = require('crypto');
        const mysign = crypto.createHash('md5').update(prestr, AlipayConfig.input_charset).digest('hex');
        // 签名结果与签名方式加入请求提交参数组中
      ...
      // 待请求参数数组
      const sPara = buildRequestPara(sParaTemp);
      let path = AlipayConfig.ALIPAY_PATH;

      for (let i3 = 0; i3 < sPara.length; i3++) {
        const obj = sPara[i3];
        const name = obj[0];
        const value = encodeURIComponent(obj[1]);
  ...
  // 构造函数，生成请求URL
  const sURL = create_direct_pay_by_user(sParaTemp);
  ...
  const rurl = 'https://' + AlipayConfig.ALIPAY_HOST + '/' + sURL;
  return rurl;
};
...
exports.paynotify = function(req, res) {
  ...
  const trade_no = req.query.trade_no; // 支付宝交易号
  const order_no = req.query.out_trade_no; // 获取订单号
  const total_fee = req.query.total_fee; // 获取总金额
  const subject = req.query.subject; // 商品名称、订单名称
  let body = '';
  if (req.query.body != null) {
    body = req.query.body; // 商品描述、订单备注、描述
  }
  const buyer_email = req.query.buyer_email; // 买家支付宝账号
  const trade_status = req.query.trade_status; // 交易状态
